...
def train_random_forest(X_train, y_train, params):
    """Train Random Forest model with optimized n_jobs for Windows."""
    # Enforce n_jobs=1 if not specified to prevent deadlocks on Windows
    n_jobs = params.get('n_jobs', 1)
    
    print(f"Starting Random Forest training (n_jobs={n_jobs})...")
    sys.stdout.flush()
